...
if __name__ == '__main__':
    ...
    # "–weights" 指定预训练权重路径；如果这里设置为空的话，就是自己从头开始进行训练；下图是官方提供的预训练权重
    # 也可以使用自己的模型
    ...
    # 这个参数在你选择yolov5l那些大一点的权重的时候，要进行适当的调整，这样才能达到好的效果。
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes')
    # 是否采用矩阵推理的方式去训练模型；
    # 所谓矩阵推理就是不再要求你训练的图片是正方形了；矩阵推理会加速模型的推理过程，减少一些冗余信息。
    parser.add_argument('--rect', action='store_true', help='rectangular training')
    # 断点续训：即是否在之前训练的一个模型基础上继续训练，default 值默认是 false；如果想采用断点续训的方式，
    # 这里我推荐一种写法，即首先将default=False 改为 default=True
    # 随后在终端中键入如下指令：
    ...
    # 遗传超参数进化；yolov5使用遗传超参数进化，提供的默认参数是通过在COCO数据集上使用超参数进化得来的（也就是下图这些参数）。
    # 由于超参数进化会耗费大量的资源和时间，所以建议大家不要动这个参数。
    ...
    # 是否启用加权图像策略，默认是不开启的；主要是为了解决样本不平衡问题；
    # 开启后会对于上一轮训练效果不好的图片，在下一轮中增加一些权重；
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')
    # 设备选择；这个参数就是指定硬件设备的，系统会自己判断的
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    # 是否启用多尺度训练，默认是不开启的；多尺度训练是指设置几种不同的图片输入尺度，
    # 训练时每隔一定iterations随机选取一种尺度训练，这样训练出来的模型鲁棒性更强。
    ...
    #DDP参数，请勿修改 DistributedDataParallel 单机多卡训练，单GPU设备不需要设置；
    ...
    # 每次预测模型的结果是否保存在原来的文件夹；如果指定了这个参数的话，
